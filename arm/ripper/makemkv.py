...
class MakeMkvRuntimeError(RuntimeError):
    ...
    def __init__(self, error):
        self.message = (
            f"Call to MakeMKV failed with code: {error.returncode} ({error.output})"
        )
...
def makemkv(logfile, job):
    ...
    cmd = (
        f"makemkvcon -r info disc:9999  |grep {job.devpath} |grep -oP '(?<=:).*?(?=,)'"
    )
    try:
        mdisc = subprocess.check_output(cmd, shell=True).decode("utf-8")
    ...
    if cfg["RIPMETHOD"] == "backup" and job.disctype == "bluray":
        # backup method
        cmd = (
            f'makemkvcon backup --decrypt {cfg["MKV_ARGS"]} '
...
def process_tracks(job, logfile, rawpath):
    ...
    for track in job.tracks:
        if track.length < int(cfg["MINLENGTH"]):
            # too short
            logging.info(
                f"Track #{track.track_number} of {job.no_of_titles}. Length ({track.length}) "
            ...
            # just right
            logging.info(
                f"Processing track #{track.track_number} of {(job.no_of_titles - 1)}. "
                f"Length is {track.length} seconds."
            )
            filepathname = os.path.join(rawpath, track.filename)
            logging.info(
                f"Ripping title {track.track_number} to {shlex.quote(filepathname)}"
            )

            cmd = (
                "makemkvcon mkv {0} -r --progress=-stdout --messages=-stdout"
                "dev:{1} {2} {3} --minlength={4}>> {5}".format(
                    cfg["MKV_ARGS"],
                    job.devpath,
...
def update_key():
    ...
    try:
        ...
        subprocess.run(
            update_cmd, capture_output=True, shell=True, check=True
        )  # noqa: F841
...
def get_track_info(mdisc, job):
    ...
    logging.info(
        "Using MakeMKV to get information on all the tracks on the disc.  This will take a few minutes..."
    )
            ...
            if msg_type == "TINFO":
                if track != line_track:
                    if line_track == int(0):
                        ...
                        utils.put_track(
                            job, track, seconds, aspect, fps, False, "makemkv", filename
                        )
                    track = line_track

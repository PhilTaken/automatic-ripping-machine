    ...
    for i in range(wait_time):  # give up after the users wait period in seconds
        try:
            ...
            if "locked" in str(e):
                time.sleep(1)
                app.logger.debug(
                    f"database is locked - trying in 1 second {i}/{wait_time} - {e}"
                )
            else:
...
def make_dir(path):
    ...
    if not os.path.exists(path):
        ...
        try:
            ...
            err = (
                "Couldn't create a directory at path: "
                + path
                + " Probably a permissions error.  Exiting"
            )
...
def get_info(directory):
    ...
    for i in os.listdir(directory):
        if os.path.isfile(os.path.join(directory, i)):
            ...
            file_list.append(
                [i, access_time, create_time, fsize]
            )  # [file,most_recent_access,created]
    return file_list
...
def call_omdb_api(title=None, year=None, imdb_id=None, plot="short"):
    ...
    if imdb_id:
        strurl = "http://www.omdbapi.com/?i={1}&plot={2}&r=json&apikey={0}".format(
            omdb_api_key, imdb_id, plot
        )
    elif title:
        # try:
        ...
        if year and year is not None:
            ...
            strurl = (
                "http://www.omdbapi.com/?s={1}&y={2}&plot={3}&r=json&apikey={0}".format(
                    omdb_api_key, title, year, plot
                )
            )
        else:
            strurl = "http://www.omdbapi.com/?s={1}&plot={2}&r=json&apikey={0}".format(
                omdb_api_key, title, plot
            )
    else:
...
def abandon_job(job_id):
    ...
    try:
        ...
        t[
            "Error"
        ] = f"couldnt find job.pid - {job.pid}"  # This is a soft error db changes still went through
    ...
    try:
        ...
        if mode == "delete" and job_id is not None:
            ...
            if job_id == "all":
                ...
                # db.session.commit()
                app.logger.debug(
                    "Admin is requesting to delete all jobs from database!!! No deletes went to db"
                )
                ...
                try:
                    ...
                    app.logger.debug(
                        "Admin requesting delete job {} from database!".format(job_id)
                    )
                except ValueError:
                    app.logger.debug(
                        "Admin is requesting to delete a job but didnt provide a valid job ID"
                    )
                    return {
                        "success": False,
                        "job": "invalid",
                        "mode": mode,
                        "error": "Not a valid job",
                    }
                else:
...
def get_x_jobs(job_status):
    ...
    if job_status in ("success", "fail"):
        ...
        jobs = (
            db.session.query(Job).filter(Job.status.notin_(["fail", "success"])).all()
        )
    ...
    if p["total_results"] > 0:
        ...
        if p["total_results"] > 0:
            ...
            for s in p["results"]:
                app.logger.debug(s)
                s["poster_path"] = (
                    s["poster_path"] if s["poster_path"] is not None else None
                )
                s["release_date"] = (
                    "0000-00-00" if "release_date" not in s else s["release_date"]
                )
                s["imdbID"] = tmdb_get_imdb(s["id"])
                s["Year"] = (
                    re.sub(TMDB_YEAR_REGEX, "", s["first_air_date"])
                    if "first_air_date" in s
                    else re.sub(TMDB_YEAR_REGEX, "", s["release_date"])
                )
                s["Title"] = (
                    s["title"] if "title" in s else s["name"]
                )  # This isnt great
                s["Type"] = "movie"
                app.logger.debug(
                    f"{s['Title']} ({s['Year']})- {poster_base}{s['poster_path']}"
                )
                ...
                app.logger.debug(
                    f"trying {search_query.capitalize()} == {s['Title'].capitalize()}"
                )
    ...
    if p["total_results"] > 0:
        ...
        if p["total_results"] > 0:
            ...
            for s in p["results"]:
                ...
                search_query_pretty = str.replace(r"\+", " ", search_query)
                app.logger.debug(
                    f"trying {search_query_pretty.capitalize()} == {s['Title'].capitalize()}"
                )
            x["Search"] = p["results"]
            return x
...
def tmdb_find(imdb_id):
    ...
    if len(p["movie_results"]) > 0:
        ...
        app.logger.debug(
            f"{s['results'][0]['title']} ({x})- {poster_base}{s['results'][0]['poster_path']}"
        )
        ...
        app.logger.debug(
            f"{s['results'][0]['name']} ({x})- {poster_base}{s['results'][0]['poster_path']}"
        )
        s["poster_url"] = f"{poster_base}{s['results'][0]['poster_path']}"
        s["Plot"] = s["results"][0]["overview"]
        s["background_url"] = f"{poster_base}{s['results'][0]['backdrop_path']}"
...
def metadata_selector(func, query="", year="", imdb_id=""):
    ...
    if cfg["METADATA_PROVIDER"].lower() == "tmdb":
        ...
        if func == "search":
            ...
            s = call_omdb_api(
                title=str(query), year=str(year), imdb_id=str(imdb_id), plot="full"
            )
            return s
...
def fix_permissions(j_id):
    ...
    if ts:
        ...
        directory_to_traverse = os.path.join(
            job.config.COMPLETED_PATH, str(job.title) + " (" + str(job.year) + ")"
        )
    try:
        corrected_chmod_value = int(str(job.config.CHMOD_VALUE), 8)
        app.logger.info(
            "Setting permissions to: "
            + str(job.config.CHMOD_VALUE)
            + " on: "
...
def process_logfile(logfile, job, r):
    ...
    job_status = re.search(
        r"Encoding: task ([0-9] of [0-9]), ([0-9]{1,3}\.[0-9]{2}) %.{0,40}"
        r"ETA ([0-9hms]*?)\)(?!\\rEncod)",
        str(line),
    )
    ...
    try:
        ...
        line = ""
    job_status_index = re.search(
        r"Processing track #([0-9]{1,2}) of ([0-9]{1,2})(?!.*Processing track #)",
        str(line),
    )
    if job_status_index:
        try:

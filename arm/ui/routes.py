...
def setup():
    ...
    if perm_file.exists():
        flash(
            str(perm_file)
            + " exists, setup cannot continue. To re-install please delete this file.",
            "danger",
        )
...
def update_password():
    ...
    if form.validate_on_submit():
        ...
        hashed = user.hash
        # our new one
        login_hashed = bcrypt.hashpw(
            str(request.form["password"]).strip().encode("utf-8"), hashed
        )
...
def database():
    ...
    return render_template(
        "database.html", jobs=jobs.items, date_format=cfg["DATE_FORMAT"], pages=jobs
    )
...
def feed_json():
    ...
    return app.response_class(
        response=json.dumps(j, indent=4, sort_keys=True),
        status=200,
...
def settings():
    ...
    """
    x = ""
    arm_cfg_file = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "../..", "arm.yaml"
    )
    ...
    return render_template(
        "settings.html", settings=cfg, form=form, raw=x, jsoncomments=comments
    )
...
def ui_settings():
    ...
    if form.validate_on_submit():
        use_icons = (
            False if str(form.use_icons.data).strip().lower() != "true" else True
        )
        save_remote_images = (
            False
            if str(form.save_remote_images.data).strip().lower() != "true"
            else True
        )
        x = {
...
def history():
    ...
    return render_template(
        "history.html", jobs=jobs.items, date_format=cfg["DATE_FORMAT"], pages=jobs
    )
...
def submitrip():
    ...
    if form.validate_on_submit():
        ...
        return redirect(
            url_for(
                "list_titles", title=form.title.data, year=form.year.data, job_id=job_id
            )
        )
...
def changeparams():
    ...
    if form.validate_on_submit():
        ...
        cfg["MAINFEATURE"] = config.MAINFEATURE = bool(
            format(form.MAINFEATURE.data)
        ...
        flash(
            f"Parameters changed. Rip Method={config.RIPMETHOD}, Main Feature={config.MAINFEATURE},"
...
def customtitle():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(
            f"custom title changed. Title={form.title.data}, Year={form.year.data}.",
            "success",
        )
        return redirect(url_for("home"))
...
def list_titles():
    ...
    if (
        ...
        or "Error" in search_results
        or ("Search" in search_results and len(search_results["Search"]) < 1)
    ):
        app.logger.debug("No results found. Trying without year")
        flash(
            f"No search results found for {title} ({year})<br/> Trying without year",
            "danger",
        )
...
def updatetitle():
    ...
    flash(
        f"Title: {job.title_auto} ({job.year_auto}) was updated to {new_title} ({new_year})",
        "success",
    )
    return redirect("/")
...
def home():
    ...
    try:
        ...
        flash(
            "There was a problem accessing the ARM folders. Please make sure you have setup ARM<br/>"
            'Setup can be started by visiting <a href="/setup">setup page</a> ARM will not work correctly until'
            "until you have added an admin account",
            "danger",
        )
    ...
    if os.path.isfile(cfg["DBFILE"]):
        try:
            jobs = (
                ...
                .filter(Job.status.notin_(["fail", "success"]))
                .all()
            )
        except Exception:
            # db isnt setup
...
def import_movies():
    ...
    for movie in movie_dirs:
        ...
        if matched:
            ...
            movie_files = [
                f
                for f in listdir(join(my_path, str(movie)))
                if isfile(join(my_path, str(movie), f))
                and f.endswith("." + dest_ext)
                or isfile(join(my_path, str(movie), f))
                and f.endswith(".mp4")
                or isfile(join(my_path, str(movie), f))
                and f.endswith(".avi")
            ]
            ...
            subfiles = [
                ...
                if isfile(join(sub_path, f))
                and not f.startswith(".")
                or isdir(join(sub_path, f))
                and not f.startswith(".")
            ]
            for sub_movie in subfiles:
                ...
                if sub_matched:
                    # This is only for pycharm
                    sub_movie_name = str.replace(
                        " ", "%20", sub_matched.group(1).strip()
                    )  # movie
                    sub_movie_name = str.replace("&", "%26", sub_movie_name)
                    p2, imdb_id = utils.get_omdb_poster(
                        sub_movie_name, sub_matched.group(2)
                    )
                    app.logger.debug(listdir(join(sub_path, str(sub_movie))))
                    # If the user selects another ext thats not mkv we are f
                    sub_movie_files = [
                        f
                        for f in listdir(join(sub_path, str(sub_movie)))
                        if isfile(join(sub_path, str(sub_movie), f))
                        and f.endswith("." + dest_ext)
                        or isfile(join(sub_path, str(sub_movie), f))
                        and f.endswith(".mp4")
                        or isfile(join(my_path, str(movie), f))
                        and f.endswith(".avi")
                    ]
    ...
    db.session.commit()
    return app.response_class(
        response=json.dumps(movies, indent=4, sort_keys=True),
        status=200,
        mimetype="application/json",
    )
...
def send_movies():
    ...
    for p in posts:
        ...
        url = (
            f"{base_url}/api/v1/?mode=p&api_key={api_key}&crc64={p.crc_id}&t={p.title}&y={p.year}&imdb={p.imdb_id}"
        ...
        if bool(req["success"]):
            ...
            i += 1
    return render_template(
        "send_movies.html", sent=r["sent"], failed=r["failed"], full=r
    )
